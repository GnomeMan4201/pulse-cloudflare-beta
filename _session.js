...
const IDLE_TTL_SECONDS = 60 * 60 * 24 * 7;      // 7 days idle window
const ABSOLUTE_TTL_SECONDS = 60 * 60 * 24 * 30; // 30 days hard cap regardless of activity
...
/**
 * Resolves the caller's session: validates absolute/idle expiry, refreshes the
 * GitHub access token server-side if it's expired (never exposing tokens to the client),
 * and slides the idle window. Returns { token, sid, reason } — reason set only on failure.
 */
